...
namespace common {
  ...
  auto do_metrics_cpu() -> asio::awaitable<void> {
    ...
    while (true) {
      ...
      cpu_metrics_bk = std::move(cpu_metrics);
    }
  }

  auto start_cpu_metrics() -> asio::awaitable<void> {
    LOG_INFO("start cpu metrics");
    asio::co_spawn(co_await asio::this_coro::executor, do_metrics_cpu(), common::exception_handle);
  }
