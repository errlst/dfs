...
static auto sm_regist_handle(std::shared_ptr<connection_t> conn, std::shared_ptr<proto_frame_t> req_frame) -> asio::awaitable<void> {
    ...
    for (auto storage : get_group_conns(calc_group(req_data.storage_info().id()))) {
        auto info = res_data.add_storage_info();
        info->set_id(storage->get_data<uint32_t>(conn_data::storage_id).value());
        info->set_port(storage->get_data<uint32_t>(conn_data::storage_port).value());
        info->set_magic(storage->get_data<uint32_t>(conn_data::storage_magic).value());
        info->set_ip(storage->get_data<std::string>(conn_data::storage_ip).value());
    }
    ...
    *res_frame = {
    ...
    /* save storage info */
    conn->set_data(conn_data::storage_id, req_data.storage_info().id());
