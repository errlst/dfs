...
//   g_m_log->log_warn(std::format("client {} disconnect", conn->to_string()));
//   co_return;
// }

// auto cm_valid_storage_handle(std::shared_ptr<connection> conn, std::shared_ptr<proto_frame_t> req_frame) -> asio::awaitable<void> {
//   /* get a valid storage */
//   auto valid_storage = std::shared_ptr<connection>{};
...
//   }
//   if (!valid_storage) {
//     g_m_log->log_warn(std::format("not find valid storage for size {}", *(uint64_t *)req_frame->data));
...
//     g_m_log->log_error("failed to serialize cm_valid_storage response");
//     co_await conn->send_res_frame(
//         proto_frame_t{
//             .cmd = req_frame->cmd,
//             .stat = 2,
...
// auto cm_group_storages_handle(std::shared_ptr<connection> conn, std::shared_ptr<proto_frame_t> req_frame) -> asio::awaitable<void> {
...
//   };
//   if (!res_data.SerializeToArray(res_frame->data, res_frame->data_len)) {
//     g_m_log->log_error("failed to serialize cm_group_storages response");
//     co_await conn->send_res_frame(
//         proto_frame_t{
//             .cmd = req_frame->cmd,
//             .stat = 2,
...
std::map<uint16_t, request_handle> client_req_handles{
