...
static auto init_conf() -> asio::awaitable<void> {
    conf.pid = getpid();
    conf.filename = std::format("{}/data/monitor.txt", g_conf["common"]["base_path"].get<std::string>());
    conf.interval = std::chrono::milliseconds{g_conf["monitor_service"]["interval"].get<uint64_t>()};
    for (auto &path : g_conf["storage_service"]["hot_paths"]) {
        conf.hot_fs.emplace_back(path.get<std::string>(), 0, 0);
    }
    for (auto &path : g_conf["storage_service"]["warm_paths"]) {
        conf.warm_fs.emplace_back(path.get<std::string>(), 0, 0);
    }
    for (auto &path : g_conf["storage_service"]["cold_paths"]) {
...
auto monitor_service() -> asio::awaitable<void> {
    g_log->log_info("monitor service start");
    co_await init_conf();
