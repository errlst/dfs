...
// //   g_s_log->log_error(std::format("master {} disconnect", master_conn->to_string()));
// //   master_conn->close();
// //   master_conn = nullptr;
// //   co_return;
// // }
...
//   /* connect to master */
//   for (auto i = 1;; i *= 2) {
//     master_conn = co_await connection::connect_to(io, conf.master_ip, conf.master_port, g_s_log);
//     if (!master_conn) {
//       g_s_log->log_error(std::format("failed to connect to master {}:{}", conf.master_ip, conf.master_port));
...
//     g_s_log->log_error("failed to serialize sm_regist request");
//     co_return false;
//   }
//   auto id = co_await master_conn->send_req_frame(std::shared_ptr<proto_frame_t>{req_frame, [](auto p) { free(p); }});
//   if (!id) {
//     g_s_log->log_error(std::format("failed to send regist to master {}", master_conn->to_string()));
//     co_return false;
//   }
...
//     g_s_log->log_error(std::format("failed to regist to master {}", master_conn->to_string()));
//     co_return false;
//   }
...
//     g_s_log->log_error("failed to parse sm_regist response");
//     co_return false;
//   }
//   storage_group_id = res_data.storage_group();
//   g_s_log->log_info(std::format("regist to master {} suc group {}", master_conn->to_string(), storage_group_id));

//   /* regist to other storage */
//   for (const auto &info : res_data.storage_info()) {
//     /* connect to storage */
//     auto conn = co_await connection::connect_to(io, info.ip(), info.port(), g_s_log);
//     if (!conn) {
//       g_s_log->log_error(std::format("failed to connect to storage {}:{}", info.ip(), info.port()));
//       continue;
//     }
//     g_s_log->log_info(std::format("connect to storage {} {} suc", conn->to_string(), info.magic()));
...
//     if (!id) {
//       g_s_log->log_error(std::format("failed regist to storage {}", conn->to_string()));
//       continue;
//     }
...
//       g_s_log->log_error(std::format("failed regist to storage {} {}", conn->to_string(), res_frame->stat));
//       continue;
//     }
...
//       g_s_log->log_info(std::format("regist to storage {} suc, conns size {}", conn->to_string(),
//                                     storage_conns.size()));
//     }
...
// std::map<proto_cmd_e, request_handle> master_req_handles{
...
//       g_s_log->log_error(std::format("unhandle cmd {} from master {}", req_frame->cmd, master_conn->to_string()));
