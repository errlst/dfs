...
static auto sync_service_timer = std::shared_ptr<asio::steady_timer>{};

auto sync_service() -> asio::awaitable<void> {
  sync_service_timer = std::make_shared<asio::steady_timer>(co_await asio::this_coro::executor);
  while (true) {
    sync_service_timer->expires_after(std::chrono::seconds{100});
    co_await sync_service_timer->async_wait(asio::use_awaitable);

    auto s_conns = registed_storages();
    LOG_INFO("sync file start, {} storages and {} files", s_conns.size(), not_synced_file_count());
    if (s_conns.empty()) {
      continue;
    }
