...
//     g_m_log->log_debug(std::format("start sync files unsynced size {}", unsync_uploaded_files.size()));
//     if (storage_conns.empty()) {
//       g_m_log->log_debug("no other storages");
//       continue;
//     }
...
//       if (!res) {
//         g_m_log->log_warn(std::format("open file {} failed", file_path));
//         continue;
//       }
...
//       auto need_sync_storages = std::set<std::shared_ptr<connection>>{};
...
//   g_m_log->log_warn(std::format("storage {} disconnect", conn->to_string()));
//   storage_conns.erase(conn);
//   co_return;
// }

// std::map<proto_cmd_e, request_handle> storage_req_handles{
...
//   g_m_log->log_debug(std::format("async file {} {}", rel_path, file_size));
...
//       g_m_log->log_warn(std::format("unhandle cmd {} from storage {}", req_frame->cmd, conn->to_string()));
//       co_await conn->send_res_frame(proto_frame_t{
//                                         .cmd = req_frame->cmd,
//                                         .stat = UINT8_MAX,
//                                     },
