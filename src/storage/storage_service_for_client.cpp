...
//   g_m_log->log_info(std::format("client {} disconnect", conn->to_string()));
//   co_return;
// }
...
//     g_m_log->log_error(std::format("invalid ss_regist data_len {}", (uint32_t)req_frame->data_len));
//     co_return;
//   }

//   if (*(uint32_t *)req_frame->data != conf.storage_magic) {
//     g_m_log->log_error(std::format("invalid ss_regist magic {} {}", conf.storage_magic, *(uint32_t *)req_frame->data));
//     co_return;
//   }
...
//     g_m_log->log_info(std::format("storage {} regist suc, storage conns size {}", conn->to_string(),
//                                 storage_conns.size()));
//   }
...
//     g_m_log->log_error(std::format("file already created {}", conn->to_string()));
...
//     g_m_log->log_error(std::format("invalid cs_create_file data_len {}", (uint32_t)req_frame->data_len));
//     co_return;
//   }
...
//   if (!file_id) {
//     g_m_log->log_error("failed to create file");
//     co_await conn->send_res_frame(
//         proto_frame_t{
//             .cmd = (uint8_t)proto_cmd_e::cs_create_file,
//             .stat = 2,
...
//   if (!file_id) {
//     g_m_log->log_warn(std::format("client {} not create file yield", conn->to_string()));
//     co_await conn->send_res_frame(
//         proto_frame_t{
...
//     g_m_log->log_error("failed to write file");
//     co_await conn->send_res_frame(
//         proto_frame_t{
//             .cmd = (uint8_t)proto_cmd_e::cs_upload_file,
//             .stat = 2,
...
//   if (!new_file_path) {
//     g_m_log->log_error("failed to close file");
//     co_await conn->send_res_frame(
//         proto_frame_t{
...
//     g_m_log->log_error(std::format("file already opened {}", conn->to_string()));
//     co_await conn->send_res_frame(
//         proto_frame_t{
...
//     g_m_log->log_error(std::format("invalid group id {}", group_id));
//     co_await conn->send_res_frame(
//         proto_frame_t{
//             .cmd = (uint8_t)proto_cmd_e::cs_open_file,
//             .stat = 2,
...
//   if (!conn->has_data(conn_data::c_open_file_id)) {
//     g_m_log->log_error(std::format("file not opened {}", conn->to_string()));
//     co_await conn->send_res_frame(
//         proto_frame_t{
...
//     g_m_log->log_error("failed to read file");
//     co_await conn->send_res_frame(
//         proto_frame_t{
//             .cmd = (uint8_t)proto_cmd_e::cs_download_file,
//             .stat = 2,
...
// std::map<proto_cmd_e, request_handle> client_req_handles{
...
//       g_m_log->log_warn(std::format("unhandle cmd {} from client {}", req_frame->cmd, conn->to_string()));
//       continue;
//     }
