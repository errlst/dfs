    ...
    while (true) {
        ...
        if (frame) {
            switch (frame->cmd) {
                ...
                // default:
                //     g_log->log_warn(std::format("unknown cmd {}", frame->cmd));
                //     break;
            }
        }
    }
...
static auto regist_to_master() -> asio::awaitable<bool> {
    ...
    *req_frame = {
        ...
        .data_len = (uint32_t)sizeof(sm_regist_req_t),
    };
    *(sm_regist_req_t *)req_frame->data = {
        .storage_id = conf.id,
        .storage_magic = conf.storage_magic,
        .storage_port = conf.port,
        .master_magic = conf.master_magic,
    };
    ...
    // for (auto p = (sm_regist_res_t *)res_frame->data; (void *)p < (void *)(res_frame->data + res_frame->data_len); p = (sm_regist_res_t *)((char *)p + sizeof(sm_regist_res_t) + p->ip_len)) {
    //     auto ip = std::string_view{p->ip, p->ip + p->ip_len};
    //     g_log->log_debug(std::format("group storage infomation {} {} {}", ip, p->storage_port, p->storage_magic));

    //     auto [conn, ec] = co_await connection_t::connect_to(co_await asio::this_coro::executor, ip, p->storage_port,
    //                                                         connection_conf_t{
    //                                                             .heartbeat_timeout = conf.heartbeat_timeout,
    //                                                             .heartbeat_interval = conf.heartbeat_interval,
    //                                                             .log = g_log,
    //                                                         });
    //     if (ec) {
    //         g_log->log_error(std::format("failed to connect to storage {}:{} {}", ip, p->storage_port, ec.message()));
    //         continue;
    //     }

    //     auto req_frame = (proto_frame_t *)malloc(sizeof(proto_frame_t) + sizeof(ss_im_storage_req_t));
    //     *req_frame = {
    //         .frame_magic = FRAME_MAGIC,
    //         .cmd = (uint8_t)proto_cmd_e::ss_im_storage,
    //         .data_len = (uint32_t)sizeof(ss_im_storage_req_t),
    //     };
    //     *(ss_im_storage_req_t *)req_frame->data = {
    //         .storage_magic = conf.storage_magic,
    //     };
    ...
    //     if (!res_frame || res_frame->cmd != (uint8_t)proto_cmd_e::ss_im_storage || res_frame->stat != 0) {
    //         g_log->log_error(std::format("failed to im storage to {} {}", conn->to_string(), res_frame->stat));
    //         continue;
    //     }

    //     storage_conns.insert(conn);
    //     g_log->log_info(std::format("connect to storage {} suc", conn->to_string()));
    // }
